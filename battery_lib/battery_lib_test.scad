...
module Generate(index = 0)
{
    ...
    if (supported)
    {
        ...
        translate([ x_offset, 0, 0 ]) BatteryLib_GenerateBatteryModel(battery_name);

        if (Display_Battery_Names)
        {
            translate([ x_offset, -BatteryLib_BodyLength(battery_name) / 2 ]) rotate([ 0, 0, 90 ])
    ...
    if (index < len(Battery_Names) - 1)
    {
        ...
        translate([ x_offset, 0, 0 ]) Generate(index + 1);
    }
}
